...
router.delete('/:id',async(req:AuthRequest,res,next)=>{try{const c=await prisma.course.findFirst({where:{id:String(req.params.id),ownerId:req.user!.id}});if(!c)return res.status(404).json({message:'Course not found'});await prisma.course.delete({where:{id:c.id}});res.status(204).end()}catch(e){next(e)}});
